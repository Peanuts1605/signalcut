...
export function App() {
  ...
  return <div className="shell">
    ...
    <aside className="inspector"><div className="aside-label">STORY UNDER REVIEW</div><h2><i style={{background:accents[data.storyboard.scenes[0].purpose]}}/>{title(strategy)}</h2><div className="score"><small>JUDGE CLARITY</small><strong>{data.receipt.clarity_score} / 6</strong><span>All clarity signals are strong.</span></div><dl><dt>Duration</dt><dd>{Math.round(data.proof.duration_seconds)} seconds</dd><dt>Scenes</dt><dd>{candidate?.beats.length??6} scenes</dd><dt>Candidate hash</dt><dd><code>{candidate?.manifest_hash.slice(0,8)}</code></dd><dt>Rendered cut</dt><dd>{title(data.storyboard.strategy)}</dd><dt>Render</dt><dd>{data.proof.width}×{data.proof.height}</dd></dl></aside>
  </div>;
}
