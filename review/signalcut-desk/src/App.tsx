import {Check, Expand, Link2, Pause, Play, Share2, Volume2, VolumeX} from "lucide-react";
...
type Data = {storyboard: Storyboard; candidates: Candidate[]; evidence: Evidence[]; receipt: Receipt; proof: Proof};
type Tab = "Evidence" | "Stories" | "Preview" | "Receipt";

const tabs: Tab[] = ["Evidence", "Stories", "Preview", "Receipt"];
...
export function App() {
  ...
  useEffect(()=>{Promise.all([
    fetch("/proof/storyboard.json").then(r=>r.json()), fetch("/proof/story-candidates.json").then(r=>r.json()),
    fetch("/proof/evidence-manifest.json").then(r=>r.json()), fetch("/proof/selection-receipt.json").then(r=>r.json()),
    fetch("/proof/render-proof.json").then(r=>r.json()),
  ]).then(([storyboard,candidates,evidence,receipt,proof])=>setData({storyboard,candidates,evidence,receipt,proof}));},[]);
  ...
  return <div className="shell">
    ...
    <main>
      ...
      <footer><span><Check/>Evidence linked</span><span><Check/>Render verified</span><span><Check/>Receipt ready</span><code>{data.storyboard.manifest_hash.slice(0,8)}</code></footer>
    </main>
...
function Preview({data,playing,muted,video,togglePlay,setMuted,seek}:{data:Data;playing:boolean;muted:boolean;video:React.RefObject<HTMLVideoElement|null>;togglePlay:()=>void;setMuted:(v:boolean)=>void;seek:(i:number)=>void}){
 return <div className="preview"><div className="screen"><video ref={video} muted={muted} onPlay={()=>{}} onEnded={()=>{}} poster="/proof/preview-poster.jpg" src="/proof/signalcut_threadloom_outcome_first.mp4"/><div className="controls"><button aria-label={playing?"Pause":"Play"} onClick={togglePlay}>{playing?<Pause/>:<Play/>}</button><button aria-label={muted?"Unmute":"Mute"} onClick={()=>{if(video.current)video.current.muted=!muted;setMuted(!muted)}}>{muted?<VolumeX/>:<Volume2/>}</button><span>55 sec · outcome first</span><button aria-label="Fullscreen" onClick={()=>video.current?.requestFullscreen()}><Expand/></button></div></div><div className="timeline">{data.storyboard.scenes.map((scene,i)=><button onClick={()=>seek(i)} key={scene.purpose} style={{borderTopColor:accents[scene.purpose]}}><small>{i+1}. {scene.purpose}</small><img src={`/proof/${scene.source_paths[0]}`} alt=""/><strong>{scene.headline}</strong></button>)}</div></div>;
}
function EvidenceView({evidence}:{evidence:Evidence[]}){return <div className="open-view"><h2>Verified evidence</h2><p>Seven real Threadloom screens. Every render scene points back here.</p><div className="evidence-grid">{evidence.map(item=><article key={item.id}><img src={`/proof/${item.local_path}`} alt={item.filename}/><div><strong>{item.filename}</strong><span>{item.purpose} · {item.width}×{item.height}</span><code>{item.sha256.slice(0,12)}</code></div></article>)}</div></div>}
